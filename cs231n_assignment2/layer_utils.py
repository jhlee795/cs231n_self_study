...
def affine_bn_relu_forward(x, w, b, gamma, beta, bn_params) : 
    affine,affine_cache = affine_forward(x,w,b)
    bn, bn_cache = batchnorm_forward(affine, gamma, beta, bn_params)
...
def affine_relu_drop_forward(x, w, b, dropout_params) : 
    ...
    affine, (affine_cache, relu_cache) = affine_relu_forward(x,w,b) 
    out, dropout_cache = dropout_forward(affine, dropout_params) 
    cache = (affine_cache, relu_cache, dropout_cache) 
...
def affine_relu_drop_backward(dout, cache) : 
    ...
    daffine = dropout_backward(dout, dropout_cache) 
    dx, dw, db = affine_relu_backward(daffine, (affine_cache, relu_cache))
    
    return dx, dw, db
    
def affine_bn_relu_drop_forward(x, w, b, gamma, beta, bn_params, dropout_params) :
    
    affine, (affine_cache, bn_cache, relu_cache) = affine_bn_relu_forward(x,w,b, gamma, beta, bn_params)
    out, dropout_cache = dropout_forward(affine, dropout_params)
...
def affine_bn_relu_drop_backward(dout, cache) : 
    ...
    dx, dw, db, dgamma, dbeta = affine_bn_relu_backward(da, (affine_cache, bn_cache, relu_cache))
